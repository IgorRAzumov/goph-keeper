...
import (
	"goph-keeper/internal/server/delivery/http/handler/auth"
	"goph-keeper/internal/server/delivery/http/handler/health"
	synchandler "goph-keeper/internal/server/delivery/http/handler/sync"
...
func Router(log logging.Logger, deps Dependencies) chi.Router {
	...
	router.Route("/api/v1", func(internalRouter chi.Router) {
		...
		internalRouter.Route("/sync", func(r chi.Router) {
			r.Use(httpmiddleware.BearerAuth(deps.Verify))
